...
In a game, press `/` or Enter (or tap the `/` button in the top bar) to
open the command bar, type `/admin` and press Enter. The panel lets you
switch to any tank (single, twin, triple, diagonal, omni, triplet, quad,
necromancer, octo) or return to AUTO, where the tank follows your kill
count. Other players see your chosen tank. The override resets when you
leave the room. There is no password: anyone can type `/admin`.
